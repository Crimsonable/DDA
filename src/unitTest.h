...
namespace DDA {
    ...
    public:
        ...
        TestFuncRetType TestForMatExpression(const int M, const int K, const int N, int numThread) {
#ifndef _DEBUG
            omp_set_num_threads(numThread);
#endif
        ...
        void Loop(int n, const int M, const int K, const int N, int threads, Func&& func) {
            double total_t = 0, total_et = 0, sum, sume;
            bool flag;
            for (int i = 0; i < n; ++i) {
                double t, et;
                auto counter = (this->*func)(M, K, N, threads);
